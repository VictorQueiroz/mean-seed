...
} ]), angular.module("Post/Service", [ "ngResource" ]).factory("Post", [ "$resource", function($resource) {
    return $resource("", {}, {
        get: {
            url: "/api/posts/:id",
            params: {
...
} ]), angular.module("Post/Ctrl/PostDetail", [ "ngRoute", "Post/Service" ]).controller("PostDetailCtrl", [ "$scope", "$routeParams", "Post", function($scope, $routeParams, Post) {
    $scope.user = Post.get({
        id: $routeParams.id
    });
} ]), angular.module("Post/Ctrl/PostList", [ "Post/Service" ]).controller("PostListCtrl", [ "$scope", "Post", function($scope, Post) {
    $scope.posts = Post.list();
} ]), angular.module("Post/Controllers", [ "Post/Ctrl/PostList", "Post/Ctrl/PostDetail" ]), 
angular.module("Post/Routes", [ "Post/Controllers" ]).config([ "$routeProvider", function($routeProvider) {
    $routeProvider.when("/posts", {
        templateUrl: "posts/list.tpl.html",
        controller: "PostListCtrl"
    });
} ]), angular.module("Post/Services", [ "ngResource", "Post/Service" ]), angular.module("Session/Service", [ "ngCookies" ]).factory("Session", [ "$http", "$cookieStore", function($http) {
