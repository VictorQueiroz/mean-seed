...
}), angular.module("app", [ "app.controllers", "app.filters", "app.directives", "app.services", "partials", "ngRoute", "ui.bootstrap", "mgcrea.ngStrap" ]).config([ "$routeProvider", "$locationProvider", function($routeProvider, $locationProvider) {
    $routeProvider.when("/", {
...
} ]), angular.module("app.controllers", [ "app.services" ]).controller("AuthCtrl", [ "$scope", "$http", "$alert", "$socket", function($scope, $http, $alert, $socket) {
    $socket.on("user authenticated", function(user) {
        alert("Congratulations, " + user.username + ", you're logged!");
...
} ]).controller("UserCtrl", [ "$scope", "User", function() {} ]), angular.module("app.directives", []), 
...
} ]), angular.module("app.services", []).value("version", "0.0.1").factory("$socket", [ "$window", function($window) {
    return $window.io();
